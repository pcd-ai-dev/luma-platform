 ...
 */
   ...
   //---------------------------------------------------------
	...
	//---------------------------------------------------------
			...
			if(empty($idGallery)){
				$reqv = "SELECT (SELECT COUNT(id) FROM ".$prefixVideo."item WHERE idCategory=:idCategory_A) as numberItem, v.*, l.*
					   FROM ".$prefixVideo."item v
					   INNER JOIN ".$prefixVideo."lang l
					   ON v.id=l.idVideo
					   AND l.idLang=:idLang
					   WHERE v.status=1
					   AND v.idCategory=:idCategory_B
					   AND url!='0'
					   ORDER BY v.globalPosition ASC LIMIT 18 OFFSET 0";
				...
				$reqv = "SELECT (SELECT COUNT(id) FROM ".$prefixVideo."item WHERE idCategory=:idCategory_A) as numberItem, v.*, l.*
					   ...
					   AND v.idCategory=:idCategory_B
					   ORDER BY v.position ASC LIMIT 18 OFFSET 0";
				$resv = $db->prepare($reqv);
				$resv->bindValue(':idGallery', $idGallery, PDO::PARAM_INT);
			}
			$resv->bindValue(':idCategory_A', $idCategory, PDO::PARAM_INT);
			$resv->bindValue(':idCategory_B', $idCategory, PDO::PARAM_INT);
			...
			if($nbv==0){
				$videoContent.= '<div class="novideolist">'.$translations['NOVIDEOTXT'].'</div>';
			}
   ...
   //---------------------------------------------------------
		...
		$req = "SELECT v.*, l.* FROM ".$prefixVideo."gallery v
		...
		for($i=0;$i<$nb;$i++){
			if($tab[$i]['idGallery']==$idGallery){$statusGallery='class="current"';}else{$statusGallery='';}
			$menuContent.='<a href="" id="'.$tab[$i]['idGallery'].'" '.$statusGallery.' >'.$tab[$i]['title'].'</a>';
		}
