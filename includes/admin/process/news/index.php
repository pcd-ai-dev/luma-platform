...
*/
	...
	//--------------------------------------------------------- 
		...
		if(isset($_POST['addPostStart'])){
			...
			$typePost=$secure->v('string', 'source', false) ?? '';

			$reqPostAdd = "INSERT INTO ".$prefixPost."item (date,idSite,idCategory,type) VALUES(:datePost,:idSite, :idCategory, :typePost)";
			...
			$resPostAdd->bindValue(':typePost', $typePost, PDO::PARAM_INT);
			$resPostAdd->execute();
