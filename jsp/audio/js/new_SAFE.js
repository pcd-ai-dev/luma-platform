(function (c) {
	...
	b.randomiseArray = function (a) {
		...
		for (c = 0; c < a; c++) b[c] = c;
		for (c = 0; c < a; c++) {
			var l = Math.round(Math.random() * (b.length - 1));
			d[c] = b[l];
			b.splice(l, 1)
		}
		return d
	...
	b.filterAllowedChars = function (a) {
		var b = a.length,
			...
			c;
		for (c = 0; c < b; c++) {
			var l = a.charAt(c).toLowerCase();
			"\\" == l && (l = "/");
			var q = l.charCodeAt(0);
			if (97 <= q &&
				122 >= q || 48 <= q && 57 >= q || 0 <= "_-".indexOf(l)) d += l
		}
		return d
	};
	...
	b.isMobile = function () {
		return b.hasPointerEvent && 1 < navigator.msMaxTouchPoints ?
			!0 : /Android|webOS|iPhone|iPad|iPod|sony|BlackBerry/i.test(navigator.userAgent)
	};
	b.isMac = function () {
	...
	b.hmsToSecondsOnly = function (a) {
		a = a.split(":");
		for (var b = 0, d = 1; 0 < a.length;) b += d * parseInt(a.pop()), d *= 60;
		return b
	};
...
(function (c, b) {
	c.AWPTooltip = function (a) {
		...
		this.setData = function () {
			l = 0 < d.find(".awp-tooltip").length ? d.find(".awp-tooltip") : b('<div class="awp-tooltip"><p class="awp-tooltip-inner"></p></div>').appendTo(d).hide()
		};
		this.show = function () {};
		this.hide = function () {
			l.hide()
		...
		this.seekbar = function (a, b, c, v) {
			if (z) return !1;
			l.addClass("awp-tooltip-num").find("p").text(AWPUtils.formatCurrentTime(c) + " / " + AWPUtils.formatDuration(v));
			c = d.offset().left;
			v = d.offset().top;
			b.offset();
			var q = b.offset().top;
			...
			if (b.hasClass("awp-tooltip-top")) {
				var K = h - u / 2;
				var N = q - v - J
			} else b.hasClass("awp-tooltip-bottom") && (K = h - u / 2, N = q - v + C + 15);
			K + u + c - y > w && (K = a - c - u);
			0 > K + c - y && (K = a - c);
			K + u + c - y > w && (K = w - u - c + y);
			l.css({
		...
		this.circle = function (a, b, c, v) {
			if (z) return !1;
			l.addClass("awp-tooltip-num").find("p").text(AWPUtils.formatCurrentTime(c) +
		...
		this.volume = function (a, b, c) {
			if (z) return !1;
			l.addClass("awp-tooltip-num").find("p").text(c + " %");
			c = d.offset().left;
			var v = d.offset().top,
				f = b.offset().left,
				q = b.offset().top,
			...
			if (b.hasClass("awp-tooltip-top"))
				if (b.hasClass("awp-vertical")) {
					var K = f - c + C / 2 - w / 2;
					var N = a - y - 10
				} else K = h - w / 2, N = q - v - y;
			else b.hasClass("awp-tooltip-bottom") ? b.hasClass("awp-vertical") ? (K = f - c + C / 2 - w / 2, N = a + y + 10) : (K = h - w / 2, N = q - v + u + 10) : b.hasClass("awp-tooltip-left") && b.hasClass("awp-vertical") && (K = f - c - w - 5, N = a - y / 2);
			l.css({
				left: parseInt(K, 10) + "px",
				top: parseInt(N, 10) + "px"
			}).show()
		...
		this.controls = function (a, b) {
			if (z) return !1;
			l.removeClass("awp-tooltip-num").find("p").text(b);
			var c = d.offset().left,
				f = d.offset().top,
				q = a.offset().left,
				m = a.offset().top,
			...
			a.hasClass("awp-tooltip-top") ? (c = q - c + u / 2 - w / 2, f = m - f - y) : a.hasClass("awp-tooltip-top-right") ? (c = q - c, f = m - f - y) : a.hasClass("awp-tooltip-top-left") ? (c = q - c + u - w, f = m - f - y) : a.hasClass("awp-tooltip-right") ? (c = q - c + u + 10, f = m - f + J / 2 - y / 2) : a.hasClass("awp-tooltip-bottom") ? (c = q - c + u / 2 - w / 2, f = m - f + J + 10) : a.hasClass("awp-tooltip-bottom-left") ? (c = q - c + u - w, f = m - f + J + 10) : a.hasClass("awp-tooltip-left") ?
				(c = q - c - 10 - w, f = m - f + J / 2 - y / 2) : (c = q - c + u / 2 - w / 2, f = m - f - y);
			l.css({
...
(function (c, b) {
	c.AWPTouchManager = function (a) {
		function f(a) {
			m || b(u).trigger("AWPTouchManager.MOUSE_MOVE", {
		...
		function d(a) {
			m || (b(a.currentTarget).off("mousemove", f).off("mouseleave", d), C.off("mouseleave", z), b(u).trigger("AWPTouchManager.MOUSE_LEAVE", {
				elem: b(a.currentTarget),
				e: a
			}))
		}
		...
		this.setData = function () {
			...
			if (!v)
				for (h = 0; h < N; h++) a[h].on("mouseenter", function (a) {
					...
					if (D.data("apmove")) D.on("mousemove", f);
					C.on("mouseleave", z);
...
(function (c, b) {
	c.AWPDownloadManager = function (a) {
		...
		b(document);
		var l = a.parent,
			q = a.settings.sourcePath,
			u, C, v, D, m, I;
		this.setData = function () {
			u = 0 < l.find("iframe.awp-dl-iframe").length ? l.find("iframe.awp-dl-iframe") : b('<iframe class="awp-dl-iframe"/>').css({
				position: "absolute",
				left: "-10000px",
				display: "none"
			}).appendTo(l);
			z && (C = l.find(".awp-download-confirm"))
		};
		this.download = function (a, b) {
			if (v) return !1;
			if ("file:" == c.location.protocol) return console.log("Downloading files locally is not possible! This requires online server connection."), !1;
			var d = a;
			var h = b;
			...
			if (z) {
				v = !0;
				if (!D) {
					for (var l = prompt("Please enter your email address where download link will be sent:");
						(!AWPUtils.validateEmail(l) || AWPUtils.isEmpty(l)) && null != l;) l = prompt("Please enter a valid email address:");
					(m = l) && (D = !0)
				}
				m ? f(m, h, d) : v = !1
			} else u.attr("src", q + "includes/dl.php?path=" +
				d + "&title=" + h)
		};
		this.setData()
	}
...
(function (c, b) {
	c.AWPShareManager = function (a) {
		...
		this.share = function (a, b, f) {
			var l = (c.screen.width - 600) / 2,
				z = (c.screen.height - 300) / 2,
				q = b.description || "",
				D = b.thumb ? AWPUtils.qualifyURL(b.thumb) : "";
			b = b.share;
			...
			} else "twitter" == a ? m = d + "//twitter.com/share?url=" + encodeURIComponent(b) + "&text=" + encodeURIComponent(f) : "googleplus" == a ? m = d + "//plus.google.com/share?url=" + encodeURIComponent(b) : "tumblr" == a && (m = d + "//www.tumblr.com/share/link?url=" + encodeURIComponent(b) + "&amp;name=" + encodeURIComponent(f) + "&amp;description=" + encodeURIComponent(q));
			m && c.open(m, "", "menubar=no,toolbar=no,resizable=yes,scrollbars=yes,width=600,height=300,left=" +
				l + ",top=" + z)
		};
		this.setData()
	}
...
(function (c, b) {
	c.AWPSoundLoader = function (a) {
		function f() {
			D.length ? (v = D.shift(), I = 0, y = v.limit || 500, d()) : b(u).trigger("AWPSoundLoader.END_LOAD", [m])
		}

		function d() {
			if ("file:" == c.location.protocol) alert("Using " + v.type + " locally is not possible! This requires online server connection!"), f();
			else {
				var a = v.type,
					d = v.path;
				"podcast" == a ? (a = C + "?url=" + encodeURIComponent(d), b.ajax({
					...
					b(c).find("image").length && b(c).find("image").attr("href") ?
						l = b(c).find("image").attr("href") : b(a.contents).find("itunes\\:image").length && b(a.contents).find("itunes\\:image").attr("href") && (l = b(a.contents).find("itunes\\:image").attr("href"));
					b(c).find("item").each(function () {
						...
						h.thumb || (d.find("image").length && d.find("image").attr("href") ? h.thumb = d.find("image").attr("href") : l && (h.thumb = l));
		...
		function l(a) {
			SC.get(a, {
				...
				if (b) console.log("Error getSoundCloudPage: " + b), f();
				else {
					if ("track" == a.kind) q(a);
						...
						for (c = 0; c < d; c++) q(a.tracks[c], !0)
					} else if (a.collection)
						for (d = a.collection.length, m.length + d >= y && (d = y - m.length, h = !0), c = 0; c < d; c++) q(a.collection[c], !0);
					a.next_href && !h ? l(a.next_href) : f()
				}
			})
		}
		...
		function q(a, b) {
			if (a.streamable && a.stream_url) {
				...
				"undefined" === typeof c.download && a.downloadable && a.download_url && (c.download =
				c.mp3.replace(/\/stream\\?/, "/download"));
				!c.title && a.title && (c.title = a.title);
				!c.album && a.album && (c.album = a.album);
				...
				"undefined" === typeof c.share && a.permalink_url && (c.share =
					a.permalink_url);
				m.push(c)
			}
		}
		var u = this,
			C = a.settings.sourcePath + "includes/ba-simple-proxy.php",
			v, D = [],
			m = [],
...
(function (c, b) {
	c.AWPPlaylistManager = function (a) {
		...
		this.setCounter = function (a, l) {
			"undefined" === typeof l && (l = !0);
			d.counter = l ? d.counter + parseInt(a, 10) : parseInt(a, 10);
			if (isNaN(d.counter)) alert("AWPPlaylistManager message: No active media, counter = " + d.counter);
				...
				if (d.randomPlay)
					...
					} else 0 > d.counter && (d.counter = d.randomArr[0], c(), d.randomArr[d.playlistItems - 1] == d.counter && (f = d.randomArr.splice(d.playlistItems -
						1, 1), d.randomArr.unshift(f)), d.counter = d.playlistItems - 1);
				else d.counter > d.playlistItems - 1 ? (d.counter = 0, b(d).trigger("AWPPlaylistManager.PLAYLIST_END_ALERT")) : 0 > d.counter && (d.counter = d.playlistItems - 1);
		...
		this.getCounter = function () {
			return d.randomPlay ?
				d.playlistSelect ? d.counter : d.randomArr[d.counter] : d.counter
		};
...
(function (c) {
	c.fn.awp = function (b) {
		function a(a) {
			...
			0 < A ? (Qa = A, A = 0) : A = Qa;
			f();
			"undefined" !== typeof a && "undefined" !== typeof Q && void 0 != pa.attr("data-awptooltip") && Q.controls(pa, 0 == A ? "UnMute" : "Mute")
		}

		function f(a) {
			"undefined" !== typeof a && (A = a);
			"undefined" !== typeof A && (t.volume = A, "undefined" !== typeof e && e.setVolume(A));
			qa && (aa ? jb.css("width", A * U + "px") : jb.css("height", A * U + "px"));
			0 == A ? qa && pa.find("i").removeClass("fa-volume-up fa-volume-down").addClass("fa-volume-off") : qa && pa.find("i").removeClass("fa-volume-off fa-volume-down").addClass("fa-volume-up");
		...
		function q(g) {
			...
			g.hasClass("awp-prev-toggle") ? x.previousMedia() : g.hasClass("awp-playback-toggle") ? x.togglePlayback() : g.hasClass("awp-next-toggle") ? x.nextMedia() : g.hasClass("awp-loop-toggle") ? (R = !R, n.setLooping(R), R ? ja.find("i").removeClass("awp-icon-color").addClass("awp-icon-rollover-color") : ja.find("i").removeClass("awp-icon-rollover-color").addClass("awp-icon-color")) :
				...
				void 0 != g.attr("data-type") && "undefined" !== typeof kb && p.share && !AWPUtils.isEmpty(p.share) && kb.share(g.attr("data-type").toLowerCase(), p, ra(n.getCounter()))
		}

		function u(a) {
			if (!k) return !1;
			a.preventDefault();
			if ("undefined" !== typeof Sa && Sa.isDrag()) return !1;
			a = c(a.currentTarget);
			var g = a.find("i");
			g.hasClass("awp-icon-color") && g.removeClass("awp-icon-color").addClass("awp-icon-rollover-color");
			"undefined" !== typeof Q && void 0 != a.attr("data-awptooltip") && (g = a.hasClass("awp-player-volume") ? 0 == A ? "UnMute" : "Mute" :
				a.attr("data-awptooltip"), Q.controls(a, g))
		}
		...
		function C(a) {
			...
			a = c(a.currentTarget);
			var g = a.find("i");
			a.hasClass("awp-random-toggle") && S || a.hasClass("awp-loop-toggle") && R || g.hasClass("awp-icon-rollover-color") && g.removeClass("awp-icon-rollover-color").addClass("awp-icon-color");
		...
		function v() {
			if (-1 != n.getCounter()) {
				var a = Z,
					c = F.children("div.awp-playlist-item[data-id=" + a + "]"),
					b = c.find(".awp-playlist-non-selected, .awp-playlist-selected");
				c.length &&
					(a = c.attr("data-id"), b.removeClass("awp-playlist-selected").addClass("awp-playlist-non-selected"), "undefined" !== typeof awpPlaylistItemEnabled && awpPlaylistItemEnabled(x, E, c, a))
			}
		}

		function D(a) {
			Ba = c.ajax({
				url: sa + "tmp/peaks/peaks.php",
				type: "post",
				data: [{
					name: "action",
		...
		function I() {
			p.peakData ? (e.backend.peaks = p.peakData, Ca = !0, e.drawBuffer(), Ta ? e.drawBuffer() : e.load(p.mp3, e.backend.peaks)) : p.peakImage ? (ea = !0, Va.empty().append('<img src="' + p.peakImage.load + '" alt="load"/>'), la.empty().append('<img src="' + p.peakImage.progress + '" alt="progress"/>'), T.css("display",
				"block"), la.width(T.width()), T.removeClass("awp-hidden").addClass("awp-visible"), Da(), e.setSkipArrayBuffer(!0), e.load(p.mp3)) : D(p.peakName || p.title)
		}

		function J() {
			Ea = c.ajax({
				url: sa + "peaks/remote_dl.php",
				type: "post",
				data: [{
					name: "id",
					value: p.mp3
		...
		function w(a) {
			...
			O && Wa();
			var g = Fb.find("#" + a);
			if (0 == g.length) return alert('Failed to select playlist! Check activePlaylist option in settings. Make sure that element: "' +
				a + '" exist in awp-playlist-list!'), Xa.css("display", "none"), V = !1;
		...
		function y(a) {
			...
			void 0 == a.attr("data-peak-load") ||
				AWPUtils.isEmpty(a.attr("data-peak-load")) || void 0 == a.attr("data-peak-progress") || AWPUtils.isEmpty(a.attr("data-peak-progress")) || (c.peakImage = {
			...
			void 0 != a.attr("data-limit") && !AWPUtils.isEmpty(a.attr("data-limit")) && AWPUtils.isNumber(a.attr("data-limit")) &&
				0 != Math.abs(parseInt(a.attr("data-limit"), 10)) && (c.limit = Math.abs(parseInt(a.attr("data-limit"), 10)));
			...
			void 0 == a.attr("data-idaudio") || AWPUtils.isEmpty(a.attr("data-idaudio")) || (c.idaudio = a.attr("data-idaudio"));
			void 0 == a.attr("data-selection") || AWPUtils.isEmpty(a.attr("data-selection")) || (c.selection = a.attr("data-selection"));
			void 0 == a.attr("data-deleteTrack") || AWPUtils.isEmpty(a.attr("data-deleteTrack")) || (c.deleteTrack = a.attr("data-deleteTrack"));
			void 0 == a.attr("data-album") || AWPUtils.isEmpty(a.attr("data-album")) || (c.album = a.attr("data-album"));
			void 0 == a.attr("data-duration") || AWPUtils.isEmpty(a.attr("data-duration")) || (c.duration = a.attr("data-duration"));
			...
			void 0 == a.attr("data-peak-dir") ||
				AWPUtils.isEmpty(a.attr("data-peak-dir")) || (c.peakdir = a.attr("data-peak-dir"));
			...
			void 0 != a.attr("data-duration") && !AWPUtils.isEmpty(a.attr("data-duration")) && AWPUtils.isNumber(a.attr("data-duration")) &&
				0 != Math.abs(a.attr("data-duration")) && (c.duration = Math.abs(a.attr("data-duration")));
			void 0 != a.attr("data-playback-rate") && (c.playbackRate = 1, !AWPUtils.isEmpty(a.attr("data-playback-rate")) && AWPUtils.isNumber(a.attr("data-playback-rate")) && 0 != a.attr("data-playback-rate") && (c.playbackRate = Math.abs(a.attr("data-playback-rate"))));
			void 0 == a.attr("data-link") || AWPUtils.isEmpty(a.attr("data-link")) || (c.link = a.attr("data-link"), c.target = "_blank", void 0 == a.attr("data-target") || AWPUtils.isEmpty(a.attr("data-target")) ||
		...
		function h() {
			...
			if (ba > B - 1) Db();
				...
				var a = W[ba],
					b = W[ba].type;
				/^audio$/.test(b) ? (X.push(a), h()) : -1 < c.inArray(b, Gb) ? mb.setData(a) : /^folder$/.test(b) ? K(b) : /^gdrive-folder$/.test(b) ? N(b) : (console.log("Wrong data-type in playlist! Type = " + b), h())
			}
		}

		function K(a) {
			if ("file:" == window.location.protocol) return console.log("Reading files from folders locally is not possible! This requires online server connection."),
				h(), !1;
			var b = W[ba],
				g = b.path.replace(/\/\//g, "/");
			a = sa + "includes/folder_parser.php";
			g = {
				dir: g,
				limit: b.limit || 1E4
			};
			b.id3 && (nb = ua = X.length - 1);
			c.ajax({
				type: "GET",
				url: a,
				data: g,
				...
				for (g = 0; g < d; g++) {
					var Y = a[g];
					if (/.mp3/.test(Y.basename)) {
						var H = c.extend(!0, {}, b);
						H.type = "audio";
						var e = H.dirurl ? H.dirurl + Y.basename : Y.fullpath;
						H.mp3 = e;
						"undefined" !== typeof H.download && AWPUtils.isEmpty(H.download) && (H.download = e);
						"undefined" !== typeof H.share &&
							AWPUtils.isEmpty(H.share) && (H.share = e);
						H.title || (H.title = Y.filename);
						X.push(H);
						ua++
					}
				}
		...
		function N(a) {
			...
			if (AWPUtils.isEmpty(t.gDriveAppId)) return console.log("gDriveAppId has not been set in settings!"), h(), !1;
			var b = W[ba];
			c.ajax({
				url: "https://www.googleapis.com/drive/v3/files?q='" +
					b.path + "'+in+parents&key=" + t.gDriveAppId,
				dataType: "jsonp"
			}).done(function (a) {
				var g, d = a.files.length,
					H = [],
					Y = [];
				for (g = 0; g < d; g++) {
					var e = a.files[g];
					/.mp3|mpeg|mpeg3/.test(e.mimeType) ? (Y.push(e), AWPUtils.keysrt(Y, "name")) : /.jpg|jpeg|png/.test(e.mimeType) && (H.push(e), AWPUtils.keysrt(H, "name"))
				}
				d = Y.length;
				for (g = 0; g < d; g++) {
					e = Y[g];
					a = c.extend(!0, {}, b);
					a.type = "audio";
					var f = "https://drive.google.com/uc?export=view&id=" + e.id;
					a.mp3 = f;
					console.log(f);
					"undefined" !== typeof a.download && AWPUtils.isEmpty(a.download) &&
						(a.download = "https://drive.google.com/uc?export=download&id=" + e.id);
					"undefined" !== typeof a.share && AWPUtils.isEmpty(a.share) && (a.share = "https://drive.google.com/open?id=" + e.id);
					H.length && !a.thumb && H[g] && (a.thumb = "https://drive.google.com/uc?export=view&id=" + H[g].id);
			...
			jsmediatags.read(a.mp3, {
				onSuccess: function (c) {
					...
					if (c) {
						...
						for (g = 0; g < d; g++) b += String.fromCharCode(c.data[g]);
		...
		function Db() {
			...
			for (a = 0; a < e; a++) {
				...
				if (fa) {
					...
					var m =
						c('<div class="' + k.origclasses + '"/>').attr({
							"data-type": p
						});
					delete k.origclasses;
					if (ca) {
						t ? t.after(m) : ma ? m.appendTo(F) : F.children("div").eq(M).before(m);
						var t = m
					} else m.appendTo(F);
					var q = c('<a class="awp-playlist-non-selected" href="#"/>').on("click", d).appendTo(m);
					if (!va) q.on("mouseenter", z).on("mouseleave", l);
					if (-1 != c.inArray("thumb", ob)) {
						...
						if (r) {
							...
							u.src = r;
							c('<span class="awp-playlist-thumb"></span>').append(u).appendTo(q)
						}
					} - 1 !=
						c.inArray("title", ob) && c('<span class="awp-playlist-title"></span>').appendTo(q);
					k.link && (r = c('<a class="awp-link" href="' + k.link + '" target="' + (k.target || "_blank") + '"><i class="fa fa-external-link"></i></a>'), q.after(r));
					
					/*
					k.download && (r = pb ? c('<a class="awp-download" href="' + k.download + '" download><i class="fa fa-download"></i></a>') : c('<a class="awp-download" href="#" title="download"><i class="fa fa-download"></i></a>'), q.after(r));
					*/
					
					/* ICI DOWNLOAD */
					if(k.deleteTrack==1){
						k.download && (r = pb ? c('<div class="itemTuneSelectionDelete" id="'+k.idaudio+'_'+k.title+'">Supprimer</div><div class="durationTrack">'+k.duration+'</div>') : c('<div class="itemTuneSelectionDelete" id="'+k.idaudio+'_'+k.title+'">Supprimer</div><div class="durationTrack">'+k.duration+'</div>'), q.after(r));
					}else{
						if(k.selection==1){			   
							k.download && (r = pb ? c('<div class="itemTuneSelection" id="'+k.idaudio+'" ><img src="/img/interface/icons/select_on.svg" alt="titre" width="25" id="imgTuneSelection_'+k.idaudio+'" rel="tooltip" title="Ajouter"/></div><div class="durationTrack">'+k.duration+'</div>') : c('<div class="itemTuneSelection" id="'+k.idaudio+'" ><img src="/img/interface/icons/select_on.svg" alt="'+k.title+'" width="25" id="imgTuneSelection_'+k.idaudio+'" rel="tooltip" title="Ajouter"/></div><div class="durationTrack">'+k.duration+'</div>'), q.after(r));
						}else{
							k.download && (r = pb ? c('<div class="itemTuneSelection" id="'+k.idaudio+'" ><img src="/img/interface/icons/select_off.svg" alt="titre" width="25" id="imgTuneSelection_'+k.idaudio+'" rel="tooltip" title="Ajouter"/></div><div class="durationTrack">'+k.duration+'</div>') : c('<div class="itemTuneSelection" id="'+k.idaudio+'" ><img src="/img/interface/icons/select_off.svg" alt="'+k.title+'" width="25" id="imgTuneSelection_'+k.idaudio+'" rel="tooltip" title="Ajouter"/></div><div class="durationTrack">'+k.duration+'</div>'), q.after(r));
						}
					}
			...
			ca || Ya ? (h = n.getCounter(), n.setPlaylistItems(B, !1), M <= h && (ma || n.reSetCounter(h + f)), wa ? (wa = !1, xa && !va && (ha = !0), n.setCounter(M, !1)) : xa && n.setCounter(M, !1), -1 != n.getCounter() && (Z = n.getCounter())) : n.setPlaylistItems(B);
			qb();
		...
		function Pa() {
			...
			if (fa) {
				...
				F.find("div.awp-playlist-item").each(function () {
					d =
						c(this).attr("data-id", a);
					e = d.find(".awp-playlist-non-selected, .awp-playlist-selected").attr("data-id", a);
					f = e.find(".awp-playlist-title");
					f.length && (b = ra(a), Za && (Ga.push(b), G[a].data.title_full = b), rb ? f.html(AWPUtils.counter(a) + sb + b) : f.html(b));
					G[a].id = a;
					a++
				});
				Ha && Ia && 0 < B && "h" == Ja.charAt(0) && ia.mCustomScrollbar("update")
			}
		}

		function Eb() {
			F.children(".awp-playlist-item").each(function () {
				if (!pb) c(this).find("a[class=awp-download]").off("click").on("click", function (a) {
					a.preventDefault();
					"undefined" !==
					typeof tb && (a = c(this).parent().attr("data-id"), a = G[a].data, "undefined" === typeof a.download || AWPUtils.isEmpty(a.download) || tb.download(a.download, a.title || "media"))
				})
			})
		}

		function ib(a) {
			var c = "h" == Ja.charAt(0) ? "x" : "y";
			ia.mCustomScrollbar({
				axis: c,
		...
		function ub() {
			...
			if (Hb) x.nextMedia();
				...
				var a = p.start || 0;
				"loop" == ab ? e.play(a) : "rewind" == ab ? (e.pause(), e.seekTo(a / e.getDuration())) : "stop" == ab && e.pause()
			}
		}

		function vb() {
			k && "undefined" !== typeof L && (fa && -1 != Z && v(), Ka(), n.reSetCounter())
		}

		function Ka() {
			Ea && (Ea.abort(), Ea = null);
			Ba && (Ba.abort(), Ba = null);
			"undefined" !== typeof e && (e.empty(),
				e.setSkipArrayBuffer(!1));
			ta.css("opacity", 0);
		...
		function Wa() {
			...
			n.reSetCounter();
			Za && yb.val(bb);
			Jb && (Aa = !1, cb.find("i").removeClass("fa-sort-alpha-asc").addClass("fa-sort-alpha-desc"));
		...
		function eb(a) {
			...
			else "number" === typeof a && (c = ra(a), rb && (c = AWPUtils.counter(a) +
				sb + c));
			Kb.html(c)
		}
		...
		function ra(a) {
			...
			a = G[a].data;
			var c = "";
			"undefined" === typeof a.artist || AWPUtils.isEmpty(a.artist) || "undefined" === typeof a.title || AWPUtils.isEmpty(a.title) ? "undefined" !== typeof a.artist || "undefined" === typeof a.title || AWPUtils.isEmpty(a.title) ? "undefined" === typeof a.artist || AWPUtils.isEmpty(a.artist) || "undefined" !== typeof a.title || (c = a.artist) : c = a.title : c = a.artist + zb + a.title;
			return c
			...
			*/
			...
			if(a.deleteTrack==1){
				...
				var c = "";
				"undefined" === typeof a.artist || AWPUtils.isEmpty(a.artist) || "undefined" === typeof a.title || AWPUtils.isEmpty(a.title) ? "undefined" !== typeof a.artist || "undefined" === typeof a.title || AWPUtils.isEmpty(a.title) ? "undefined" === typeof a.artist || AWPUtils.isEmpty(a.artist) || "undefined" !== typeof a.title || (c = a.artist) : c = a.title : c =  a.artist+" - "+a.album+" - "+a.title;
				return c
			}else{

				var c = "";
				"undefined" === typeof a.artist || AWPUtils.isEmpty(a.artist) || "undefined" === typeof a.title || AWPUtils.isEmpty(a.title) ? "undefined" !== typeof a.artist || "undefined" === typeof a.title || AWPUtils.isEmpty(a.title) ? "undefined" === typeof a.artist || AWPUtils.isEmpty(a.artist) || "undefined" !== typeof a.title || (c = a.artist) : c = a.title : c =  a.title;
				return c
			}
		...
		var t = c.extend({}, {
			...
			r = c(this).show(),
			Fb = c(t.playlistList).hide(),
			Lb = r.find(".awp-playlist-holder"),
			ia = r.find(".awp-playlist-inner"),
			F = r.find(".awp-playlist-content"),
			fb = r.find(".awp-playlist-filter-msg"),
			cb = r.find(".awp-sort-alpha"),
			Xa = r.find(".awp-preloader");
		r.find(".awp-player-holder");
		var Ib = r.find(".awp-player-thumb"),
			ya = r.find(".awp-media-time-current"),
			za = r.find(".awp-media-time-total"),
			Kb = r.find(".awp-media-title");
		r.find(".awp-player-controls");
		var db = r.find(".awp-playback-toggle"),
			ja = r.find(".awp-loop-toggle"),
			pa = r.find(".awp-player-volume"),
			ka = r.find(".awp-random-toggle");
		r.find(".awp-volume-wrapper");
		var oa = r.find(".awp-volume-seekbar"),
			da = r.find(".awp-volume-bg"),
			jb = r.find(".awp-volume-level"),
			La = r.find(".awp-waveform"),
			ta = r.find(".awp-waveform-preloader"),
			T = r.find(".awp-waveform-img"),
			Va = r.find(".awp-waveform-img-load"),
			Ab = r.find(".awp-waveform-img-progress-wrap"),
			la = r.find(".awp-waveform-img-progress"),
			va = AWPUtils.isMobile();
		AWPUtils.canPlayMp3();
		AWPUtils.canPlayWav();
		var sa = t.sourcePath,
			E = AWPUtils.filterAllowedChars(t.instanceName),
			na = t.activePlaylist,
			ha = t.autoPlay,
			Ta = t.drawWaveWithoutLoad,
			R = t.loopingOn,
			S = t.randomPlay,
			rb = t.useNumbersInPlaylist,
			sb = t.numberTitleSeparator,
			zb = t.artistTitleSeparator,
			A = t.volume,
			Ha = t.usePlaylistScroll,
			Ja = t.playlistScrollOrientation.toLowerCase(),
			Mb = t.useKeyboardNavigationForPlayback,
			Hb = t.autoAdvanceToNextMedia,
			ab = t.mediaEndAction,
			ob = t.playlistItemContent.split(","),
			fa = Lb.length,
			Za = r.find(".awp-search-filter").length,
			Jb = cb.length,
			wb = ya.length || za.length,
			qa = oa.length,
			k = !1,
			V = !1,
			x = this;
		...
		var Nb = c(window),
			Ob = c(document),
			Bb = AWPUtils.isIE(),
			gb, pb = AWPUtils.hasDownloadSupport(),
			Ua, ea, Ca, Ea, Ba, ma, M, ca, xa, wa, Ya, ba = -1,
			G = [],
			X = [],
			W = [],
			B = 0,
			...
			p, Ra, Ia, Aa = !1,
			Ga = [],
			ua, nb, Gb = ["podcast", "soundcloud", "soundcloud_query"];
		...
		va && (ha = !1);
		if (Bb && 12 > Bb) T.on("click", function (a) {
			if (ea) {
		...
		if (Za) var yb = r.find(".awp-search-filter").on("focus", function (a) {
				c(this).val() == bb && c(this).val("")
			}).on("blur", function () {
				AWPUtils.isEmpty(c(this).val()) && c(this).val(bb)
			}).on("keyup.apfilter", function () {
				if (0 == Ga.length) return !1;
				var a = G.length,
					b = c(this).val();
				b = Ga.filter(/./.test.bind(new RegExp(b, "i")));
				var d, e = 0;
				for (d = 0; d < a; d++) {
					var f = ra(d); - 1 < c.inArray(f, b) ? F.children("div.awp-playlist-item").eq(d).show() : (F.children("div.awp-playlist-item").eq(d).hide(), e++)
				}
				0 < a && fb.length && (e == a ? fb.show() : fb.hide());
				Ha && 0 < B && "h" == Ja.charAt(0) && ia.mCustomScrollbar("update");
		...
		var Q = new AWPTooltip({
			...
			tb = new AWPDownloadManager({
				settings: t,
				parent: r
			}),
			kb = new AWPShareManager({
				settings: t
			}),
		...
		c(n).on("AWPPlaylistManager.COUNTER_READY", function () {
			...
			if (fa) {
				...
				var a = n.getCounter(),
					c = F.children("div.awp-playlist-item[data-id=" + a + "]"),
					b = c.find(".awp-playlist-non-selected, .awp-playlist-selected");
				c.length && (Z = a, b.removeClass("awp-playlist-non-selected").addClass("awp-playlist-selected"), Ha && 0 < B && (Ra || ("h" == Ja.charAt(0) ? ia.mCustomScrollbar("scrollTo", parseInt(c.position().left), {
					scrollInertia: 500
				}) : ia.mCustomScrollbar("scrollTo", parseInt(c.position().top), {
					scrollInertia: 500
				})), Ra = !1), "undefined" !== typeof awpPlaylistItemDisabled && awpPlaylistItemDisabled(x, E, c, a))
			}
			a = n.getCounter();
			p = G[a].data;
			L = p.type;
		...
		var hb = [];
		qa && (oa.data("apmove", !0), hb.push(oa));
		if (hb.length) {
			var Sa = new AWPTouchManager(hb);
			c(Sa).on("AWPTouchManager.DRAG_START", function (a, c) {}).on("AWPTouchManager.DRAG_MOVE AWPTouchManager.DRAG_RELEASE", function (a, c) {
				if (!k) return !1;
				var b = c.elem,
					d = c.point.pageX,
					g = c.point.pageY;
				if (b.hasClass("awp-volume-seekbar")) {
					...
					f();
					"undefined" !== typeof Q && b.is('[class*="awp-tooltip"]') && (b = aa ? d - da.offset().left : g - da.offset().top, 0 > b ? b = 0 : b > U && (b = U), b = Math.max(0, Math.min(1, b / U)), aa || (b = 1 - b), b = parseInt(100 * b, 10), Q.volume(c.point, oa, b))
				}
				...
				b.hasClass("awp-volume-seekbar") &&
					"undefined" !== typeof Q && b.is('[class*="awp-tooltip"]') && (b = aa ? d - da.offset().left : g - da.offset().top, 0 > b ? b = 0 : b > U && (b = U), b = Math.max(0, Math.min(1, b / U)), aa || (b = 1 - b), b = parseInt(100 * b, 10), Q.volume(a, oa, b))
		...
		if (qa) {
			var aa = oa.hasClass("awp-vertical") ? !1 : !0,
				Qa = .5,
				U = void 0 != t.volumeSize ? t.volumeSize : aa ? da.width() : da.height();
			0 > A ? A = 0 : 1 < A && (A = 1);
			0 != A && (Qa = A)
		}
		if (Mb) Ob.on("keyup.apnav", function (b) {
			if (!k) return !1;
			var d =
				b.keyCode;
			c(b.target);
		...
		var Cb = [r.find(".awp-next-toggle"), r.find(".awp-prev-toggle"), r.find(".awp-playlist-toggle"), r.find(".awp-share-item"), cb, pa, db, ja, ka],
			Pb = Cb.length,
			Na;
		for (Na = 0; Na < Pb; Na++) {
			var Qb = c(Cb[Na]).css("cursor", "pointer").on("click", q);
			if (!va) Qb.on("mouseenter", u).on("mouseleave", C)
		}
		...
		e.on("loading", function (a) {
			a = "Chargement... " + a.toString() + "%";
			ta.css("opacity", 1).html(a)
		});
		...
		e.on("audioprocess", function (a) {
			a = e.getCurrentTime();
			var b = e.getDuration();
			wb && (ya.html(AWPUtils.tCurrentTime(a)), za.html(AWPUtils.formatDuration(b)));
			ea && Ab.width(T.width() *
		...
		e.on("seek", function (a) {
			ea && Ab.width(T.width() * a)
		});
		e.on("ready", function () {
			e.setVolume(A);
			var a = p.start || 0;
			if (p.end) var b = p.end;
			p.playbackRate && e.setPlaybackRate(p.playbackRate);
			ha && (b ? e.play(a, b) : e.play(a));
			ha = !0
		});
		e.on("waveform_ME_noPeaks", function () {
			if (Ua) {
				var a = e.backend.getPeaks(900);
				a.length && m(a, p.peakName || p.title);
				Ua = !1
			}
			ta.css("opacity", 0);
		...
		e.on("redraw", function () {
			Ca && (ta.css("opacity", 0), La.removeClass("awp-hidden").addClass("awp-visible"), Da(), Ca = !1)
		});
		this.getTitleFormatted = function (a, b) {
			...
			"undefined" === typeof a.artist || AWPUtils.isEmpty(a.artist) || "undefined" === typeof a.title || AWPUtils.isEmpty(a.title) ?
				"undefined" !== typeof a.artist || "undefined" === typeof a.title || AWPUtils.isEmpty(a.title) ? "undefined" === typeof a.artist || AWPUtils.isEmpty(a.artist) || "undefined" !== typeof a.title || (c = a.artist) : c = a.title : c = a.artist + d + a.title ;
			return c
		};
		Nb.resize(function () {
			if (!k) return !1;
		...
		this.loadPlayMedia =
			function () {
				if (!k || !L || !p) return !1;
				p.mp3 && e.backend.peaks && e.load(p.mp3, e.backend.peaks, !0);
				P = !0
			};
		...
		this.togglePlayback = function () {
			if (!k || !L) return !1;
			"undefined" !== typeof e && (!Ma && Ta && e.backend.peaks ? (ha = !0,
				x.loadPlayMedia()) : e.playPause())
		};
		this.stop = function () {
			if (!k || !L) return !1;
		...
		this.loadPlaylist = function (a) {
			if (!k || V) return !1;
			if ("undefined" === typeof a || AWPUtils.isEmpty(a)) return alert("loadPlaylist method requires id parameter. loadPlaylist failed."), !1;
			if (na == a) return !1;
			na = a;
			w(na)
		};
		this.addTrack = function (a, b, d, e) {
			...
			if ("string" !== typeof b && "[object Object]" !== Object.prototype.toString.call(b))
				if ("[object Array]" === Object.prototype.toString.call(b)) d = b.length, g = !0;
			...
			for (e = 0; e < d; e++) {
				...
				if ("html" == a) {
					f = c(f);
					var l = c("<div>").append(f.clone()).html();
		...
		this.removeTrack = function (a) {
			...
			G[a] ? (F.children(".awp-playlist-item").eq(a).remove(),
				G.splice(a, 1), Pa(), 0 < B ? (b = n.getCounter(), a == b ? (Ka(), n.setPlaylistItems(B)) : (n.setPlaylistItems(B, !1), a < b && n.reSetCounter(n.getCounter() - 1)), -1 != n.getCounter() ? (Z = n.getCounter(), eb(Z)) : awpMediaEmpty(x, E)) : Wa()) : alert('RemoveTrack with id "' + a + '" failed.')
		};
		this.sort = function (a, b) {
			if (!k || !O) return !1;
			if ("undefined" === typeof a) return console.log("Sort method requires type parameter. Sort method failed."), !1;
			var d = F.children("div.awp-playlist-item"),
				e = d.length,
				g = a.toLowerCase();
			if (2 > e) return !1;
			if ("title" ==
				g)
				for (AWPUtils.keysrt2(G, "data", "title_full", b || !1), e = [], g = 0; g < B; g++) e.push(G[g].id);
			else if ("random" == g) e = AWPUtils.randomiseArray(e), G = AWPUtils.sortArray(G, e);
		...
		this.setPlaybackRate = function (a) {
			if (!k || "undefined" !== typeof n && -1 == n.getCounter()) return !1;
			"undefined" !== typeof e && e.setPlaybackRate(a)
		};
		this.toggleRandom = function (a) {
			if (!k || "undefined" === typeof n) return !1;
		...
		this.destroyPlaylistScroll = function () {
			if (!k) return !1;
			ia.mCustomScrollbar("destroy");
			Ia = !1
		};
		this.updatePlaylistScroll = function () {
			if (!k) return !1;
			ia.mCustomScrollbar("update")
		};
		...
		this.getActiveItemId = function () {
			return k ? "undefined" !== typeof n ? n.getCounter() : null : !1
		};
		this.getPlaylistLength = function () {
			return k ? AWPUtils.isNumber(B) ? B : 0 : !1
		};
